...
function UserCreateForm() {
  ...
  const { uniqueUsername, uniqueCellPhone } = useSelector(
    (state) => state.user
  );
  ...
  const handleCellPhone = (e) => {
    ...
    if (cellphone?.length > 1) {
      dispatch(checkUniqueCellPhone(e?.target?.value));
    }
  };
        ...
        <Form onSubmit={onSubmit}>
          <Row className="gy-4">
            ...
            <Row className="align-items-center justify-content-center pe-0 mt-3">
              ...
              {uniqueCellPhone && (
                <p className="text-danger">Cellphone already exists!</p>
              )}
            </Row>
            <Col md={12}>
              <div className="mb-1">
