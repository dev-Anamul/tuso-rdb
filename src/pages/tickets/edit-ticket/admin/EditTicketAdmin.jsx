...
function CallCenterTicketEditForm() {
  ...
  useEffect(() => {
    ...
    if (teamID) {
      dispatch(getMembersByTeamId(teamID));
    }
  }, [dispatch, firstLevelCategoryID, secondLevelCategoryID, teamID]);
  ...
  useEffect(() => {
    if (ticketData) {
      ...
      SetThirdLevelCategoryID(ticketData.incidents?.thirdLevelCategoryID || "");
      setAssignedTo(ticketData.incidents?.assignedTo || "");
            ...
            <Col md={12}>
              <FormGroup>
                <div className="d-flex justify-content-between align-items-center">
                  ...
                  {!isValidImg && <p className="text-danger">Invalid</p>}
                </div>
                <input
                  id="screenshot"
              ...
              {ticketData?.incidents?.hasImg ? (
                <a
                  href={`${process.env.REACT_APP_API_URL}/tuso-api/screenshot/key/${ticketData?.incidents?.oid}`}
                  target="_blank"
                  rel="noreferrer"
                >
                  Attachment
          ...
          <Row className="">
            <div className="d-flex mt-4 justify-content-start">
              <Button
                ...
                type="submit"
                onClick={onSubmit}
                disabled={ticketData.incidents?.assignedTo}
              >
