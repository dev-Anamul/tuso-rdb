...
function PrivateRouter({ pageId }) {
  ...
  const [isModuleApproved, setIsModuleApproved] = React.useState({});
  ...
  useEffect(() => {
    const approvedModule = approvedModules?.find(
      (module) =>
        module.modules?.moduleName.toLowerCase() === pageId.toLowerCase()
    );
    setIsModuleApproved(approvedModule);
  }, [pageId]);

  // ! returns actual module name from url
